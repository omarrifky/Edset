...
userSchema.pre("save", async function () {
  //Before saving when creating user the password is hashed
...
userSchema.methods.toJSON = function () {
  // Return the user info as JASON without unwanted fields
  const user = this;
...
userSchema.methods.generateAuthToken = function () {
  // Generated the user token to access the website when registering and login
  const user = this;
  const access = "auth";
...
userSchema.methods.removeToken = function (token) {
  // Remove token after the user logs out
  const user = this;

...
export default function OrdersScreen({ navigation }) {
  const { user, token, orders, setOrders, refetchOrder, setRefetchOrder } = useContext(AuthContext);
        ...
        <View style={styles.body}>
          {orderData.map(({ ordernumber, price, products, status, _id }) => (
            <AccordionItem title={`Order #${ordernumber}`}>
              {products.map(product => <OrderProductCard navigation={navigation} productData={product} order_id={_id} />)}
              <View style={styles.totalHolder}>
                <Text style={styles.total}>EGP {price}</Text>
              </View>
              {checkCancelAll[_id] !== cancel_cases.NO_PENDING && (<Pressable style={styles.cancelBtn} onPress={() => handleCancel(_id)}>
                <Text style={styles.cancelBtnTxt}>{checkCancelAll[_id] === cancel_cases.SOME_PENDING ? 'Cancel the rest' : 'Cancel order'}</Text>
              </Pressable>)}
            </AccordionItem>
          ))}
        </View>
